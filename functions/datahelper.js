...
exports.getScoreChange = function(current, previous) {
    ...
    if ((!scoreChange.isInsert && !scoreChange.isDelete) && (
        current.crop.dataHash !== previous.crop.dataHash || 
        current.crop.title !== previous.crop.title)) {
        scoreChange.isChange = true;
...
exports.getCmsCropChange = function(current, previous) {
    ...
    if (!(flMetaProperty in doc) || 
        !doc[flMetaProperty] || 
        doc[flMetaProperty].schema !== 'crop') {
        return null;
    } 
    ...
    if (!cropChange.isInsert && !cropChange.isDelete) {
        ...
        cropChange.isChange = (current.status !== previous.status) || 
...
exports.fetchSheetValues = function(sheetTitle, apiauth, sheetId, apiKey, handler) {
    ...
    return gsheets.spreadsheets.values.get(sheetRequest)
        .then(result => {
